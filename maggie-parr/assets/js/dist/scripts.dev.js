...
(function ($) {
  ...
  $(document).ready(function () {
    ...
    function affixNav(el, elHeight, elOffset) {
      ...
      $(affixElementDesktop + ', ' + affixElementMobile).removeData('bs.affix').removeClass('affix affix-top');
      $(el).affix({
        offset: {
          top: $(elOffset).outerHeight(true) || 0
        }
      });
      $('.affix, .affix-top').wrap('<div class="affix-wrapper"></div>').parent().css('min-height', $(elHeight).outerHeight(true) || 0);
    }
  ...
  $(document).ready(function () {
    // Settings
    var topHeaderHeight = $('.navbar-header').outerHeight(true),
        // -------	Get Height of Element that is not fixed and not being changed — used for Delay before Element starts changing
    objectStyles = {
      navbarPadTop: {
        // --------------------------------------------	Custom Element/Object Name — type what you want
        element: '.navbar .navbar-collapse',
        // ------------------	CSS Class of Element that is being changed
        style: 'padding-top',
        // -------------------------------	CSS Style that is being changed
        start: 'currentValueFromCSS',
        // -----------------------	Change from e.g. 30 (Pixels) — if a String/Text is entered then the current Value from CSS File is being taken
        end: 0,
        // -------------------------------------------	Change to e.g. 0 (Pixels)
        distance: 300,
        // -----------------------------------------	Element is being resized for e.g. 300 (Pixels) scrolled
    ...
    function destroy() {
      $.each(objectStyles, function (obj, prop) {
        $(prop.element).css(prop.style, '').removeClass('animate animate-after');
      });
    }
      ...
      $.each(objectStyles, function (obj, prop) {
        if (scrolledFromTop > prop.delay) {
          ...
          if (prop.scrolled <= prop.distance) {
            ...
            $(prop.element).css(prop.style, prop.end + 'px').addClass('animate-after');
          }
  ...
  var aosEnable = true,
      // ------------------------------------------------	Turn on/off AOS
  aosMobileDisable = false,
      // -----------------------------------------------	Turn on/off AOS on Mobile
  aosContainer = 'main',
      // ----------------------------------------------	Container (e.g. 'body' [with Footer] or '#main'/'main' [without Footer])
  aosItems = '[class*="col-"], .isotope .item, .posts .post',
      // -----	Add/remove Elements to be animated
  ...
  function disableAOS(aosItem, stayEnabled) {
    ...
    aosEnable = stayEnabled || false;
  }

  if (aosEnable && (!aosMobileDisable || cssBreakpoint('md'))) enableAOS();else aosEnable = false;
  $(window).resize(debounce(function () {
    if (aosEnable && !aosEnable && (!aosMobileDisable || cssBreakpoint('md'))) {
      enableAOS();
      $(aosContainer + ' ' + aosItems).addClass('aos-animate');
    } else if (aosEnable && aosMobileDisable && cssBreakpoint('xs')) disableAOS($(aosItems));
  }));
  $('a[data-filter]').click(function () {
    if (aosEnable) disableAOS($(aosItems));
  });
  /*===================================================================================*/
